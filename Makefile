...
.PHONY: exec
exec:				##@docker Run container and execute command. (CMD='')
	@echo -n "Exec:"
	@if $(VERBOSE); \
	    then \
	        docker-compose $(PROJECT_FLAG)-f $(YML) run --rm $(SERVICE) $(CMD); \
	    else \
	        docker-compose $(PROJECT_FLAG)-f $(YML) run --rm $(SERVICE) $(CMD) 1>/dev/null 2> temp.log || touch temp.errors; \
	    fi;
	@if test -e temp.errors; then echo -e "$(ERROR_STRING)" && cat temp.log; elif test -s temp.log; then echo -e "$(WARN_STRING)" && cat temp.log; else echo -e "$(OK_STRING)"; fi; rm -f temp.errors temp.log
